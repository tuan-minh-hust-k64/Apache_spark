...
public class SparkML {
    public static void main(String[] args) {
        ...
        Dataset<Row> rawDataset = sparkSession.read().option("header", true).option("inferSchema", true).csv("src/main/resources/kc_house_data.csv");
        StringIndexer stringIndexer = new StringIndexer();
        stringIndexer.setInputCols(new String[] { "condition", "grade", "zipcode", "waterfront" })
                .setOutputCols(new String[] { "conditionIndex", "gradeIndex", "zipcodeIndex", "waterfrontIndex" });
        rawDataset = stringIndexer.fit(rawDataset).transform(rawDataset);
